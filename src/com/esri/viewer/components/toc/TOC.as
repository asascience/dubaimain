...
package com.esri.viewer.components.toc
{
	...
	public class TOC extends Tree
	{
		...
		public function TOC( map:Map = null )
		{
			...
			// Set default styles for picture in TOC 
			...
			setStyle("layerIcon", null);
			setStyle("groupLayerIcon", null);
			setStyle("mapServiceIcon", null);
		}
		...
		[Embed(source="assets/images/layer.png")]
		private var _layerIcon:Class;
		
		[Embed(source="assets/images/group-layer.png")]
		private var _groupLayerIcon:Class;
		
		[Embed(source="assets/images/map-service.png")]
		...
		//to fix non-display of scroll bar
		/*override public function validateSize(recursive:Boolean = true):void {
			super.validateSize(recursive);
			if (!initialized) return;
			if (height < measuredHeight) this.verticalScrollPolicy = ScrollPolicy.ON;
			else this.verticalScrollPolicy = ScrollPolicy.OFF;
		}*/
		...
		private function registerMapLayer( layer:Layer ):void
		{
			...
			if(tocItem.children)
			{
				...
				var t:Timer = new Timer(580,2);
				t.addEventListener(TimerEvent.TIMER_COMPLETE, endTOC);
				t.start();
			}
			function endTOC(event:TimerEvent):void
			{
				_tocRoots.addItemAt(tocItem, 0);
			}
		}
